...
/// <summary>
/// ViewModel for the t-SNE (t-distributed Stochastic Neighbor Embedding) plot visualization.
/// </summary>
public partial class TsnePlotViewModel : ViewModelBase
{
    ...
    // t-SNE parameters with defaults
    [ObservableProperty]
    private double _perplexity = 30.0;

    [ObservableProperty]
    private double _learningRate = 200.0;

    public TsnePlotViewModel(
        ...
        HoverDelayService hoverDelayService)
    {
        ...
        _messenger.Register<StudentHoverMessage>(this, (r, m) =>
        {
            ...
            if (m.StudentId == null || m.Source != "tsne")
            {
    ...
    /// <summary>
    /// Called when Perplexity changes - debounce regeneration.
    /// </summary>
    partial void OnPerplexityChanged(double value)
    {
        DebouncedRegenerate();
    }

    /// <summary>
    /// Called when LearningRate changes - debounce regeneration.
    /// </summary>
    ...
    /// <summary>
    /// Debounced regeneration to avoid regenerating on every slider tick.
    /// </summary>
    private void DebouncedRegenerate()
    {
        if (_seriesData.Count == 0 || _displayWidth == 0)
            return;

        _regenerateCts?.Cancel();
        _regenerateCts = new CancellationTokenSource();
        var token = _regenerateCts.Token;
    ...
    /// <summary>
    /// Generates the t-SNE plot with the given data.
    /// </summary>
        ...
        ThemeName theme = ThemeName.DarkMode)
    {
        ...
        try
        {
            ...
            var (svgContent, dataPoints) = _dimRedService.GenerateTsnePlot(

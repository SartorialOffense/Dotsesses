...
public partial class UmapPlotControl : UserControl
{
    ...
    public UmapPlotControl()
    {
        ...
        // Add button handlers
        CopyUmapPlotButton.Click += OnCopyClick;
    ...
    private void OnRenderWithThemeMessage(object recipient, RenderWithThemeMessage message)
    {
        ...
        CopyUmapPlotButton.IsVisible = _currentTheme == ThemeName.DarkMode;
        ...
        {
            if (DataContext is UmapPlotViewModel vm)
            {
                var width = UmapPlotArea.Bounds.Width;
                var height = UmapPlotArea.Bounds.Height;
                if (width > 0 && height > 0)
                {
    ...
    {
        if (DataContext is UmapPlotViewModel vm && !string.IsNullOrEmpty(vm.SvgContent))
        {
            RenderPointsAsShapes();
        }
    }
    ...
    private void OnSizeChanged(object? sender, SizeChangedEventArgs e)
    {
        ...
        if (DataContext is UmapPlotViewModel vm && !string.IsNullOrEmpty(vm.SvgContent))
        {
            var plotBounds = UmapPlotArea.Bounds;
        ...
        Task.Delay(150, token).ContinueWith(t =>
        {
            if (!t.IsCanceled)
            {
                Dispatcher.UIThread.Post(() =>
                {
                    if (!token.IsCancellationRequested && DataContext is UmapPlotViewModel viewModel)
                    {
                        var plotBounds = UmapPlotArea.Bounds;
                        ...
                        {
                            Console.WriteLine($"[UmapPlot] Error regenerating plot: {ex.Message}");
                        }
                    }
                });
            }
    ...
    {
        if (sender is not UmapPlotViewModel vm) return;

        if (e.PropertyName == nameof(UmapPlotViewModel.SvgContent))
        {
            UpdateSvgDisplay(vm.SvgContent);
        }
        else if (e.PropertyName == nameof(UmapPlotViewModel.HoveredStudentId))
        {
            UpdateHoverVisualization(vm);
        }
    }
    ...
    private void UpdateSvgDisplay(string? svgContent)
    {
        ...
        try
        {
            var tempPath = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "dotsesses_umap.svg");
            ...
            Dispatcher.UIThread.Post(() =>
            {
                if (DataContext is UmapPlotViewModel viewModel)
                {
        ...
        {
            Console.WriteLine($"[UmapPlot] Error loading SVG: {ex.Message}");
        }
    }

    private void RenderPointsAsShapes()
    {
        if (DataContext is not UmapPlotViewModel vm)
            return;
    ...
    private void UpdateHoverVisualization(UmapPlotViewModel vm)
    {
        RenderPointsAsShapes();
    ...
    {
        var position = e.GetCurrentPoint(UmapPlotArea);
        ...
        else
        {
            ...
            {
                WeakReferenceMessenger.Default.Send(new StudentHoverMessage(null, "umap"));
                e.Handled = true;
            }
        }
    }

...
public partial class PcaPlotControl : UserControl
{
    ...
    public PcaPlotControl()
    {
        ...
        // Add button handlers
        CopyPcaPlotButton.Click += OnCopyClick;
    ...
    private void OnRenderWithThemeMessage(object recipient, RenderWithThemeMessage message)
    {
        ...
        CopyPcaPlotButton.IsVisible = _currentTheme == ThemeName.DarkMode;
        LoadingOverlay.IsVisible = false; // Hide loading during export

        // Re-render all visual elements with new theme
        Dispatcher.UIThread.Post(() =>
        {
            if (DataContext is PcaPlotViewModel vm)
            {
                var width = PcaPlotArea.Bounds.Width;
                var height = PcaPlotArea.Bounds.Height;
                if (width > 0 && height > 0)
                {
    ...
    {
        if (DataContext is PcaPlotViewModel vm && !string.IsNullOrEmpty(vm.SvgContent))
        {
            RenderPointsAsShapes();
        }
    }
    ...
    private void OnSizeChanged(object? sender, SizeChangedEventArgs e)
    {
        ...
        if (DataContext is PcaPlotViewModel vm && !string.IsNullOrEmpty(vm.SvgContent))
        {
            var plotBounds = PcaPlotArea.Bounds;
        ...
        Task.Delay(150, token).ContinueWith(t =>
        {
            if (!t.IsCanceled)
            {
                Dispatcher.UIThread.Post(() =>
                {
                    if (!token.IsCancellationRequested && DataContext is PcaPlotViewModel viewModel)
                    {
                        var plotBounds = PcaPlotArea.Bounds;
                        ...
                        {
                            Console.WriteLine($"[PcaPlot] Error regenerating plot: {ex.Message}");
                        }
                    }
                });
            }
    ...
    {
        if (sender is not PcaPlotViewModel vm) return;

        if (e.PropertyName == nameof(PcaPlotViewModel.SvgContent))
        {
            UpdateSvgDisplay(vm.SvgContent);
        }
        else if (e.PropertyName == nameof(PcaPlotViewModel.HoveredStudentId))
        {
            UpdateHoverVisualization(vm);
        }
    }
    ...
    private void UpdateSvgDisplay(string? svgContent)
    {
        ...
        try
        {
            var tempPath = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "dotsesses_pca.svg");
            ...
            Dispatcher.UIThread.Post(() =>
            {
                if (DataContext is PcaPlotViewModel viewModel)
                {
        ...
        {
            Console.WriteLine($"[PcaPlot] Error loading SVG: {ex.Message}");
        }
    }

    private void RenderPointsAsShapes()
    {
        if (DataContext is not PcaPlotViewModel vm)
            return;
    ...
    private void UpdateHoverVisualization(PcaPlotViewModel vm)
    {
        RenderPointsAsShapes();
    ...
    {
        var position = e.GetCurrentPoint(PcaPlotArea);
        ...
        else
        {
            ...
            {
                WeakReferenceMessenger.Default.Send(new StudentHoverMessage(null, "pca"));
                e.Handled = true;
            }
        }
    }
